...
```bash
lambda-dynamo$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
lambda-dynamo$ sam local start-api
lambda-dynamo$ curl http://localhost:3000/
```
...
```bash
lambda-dynamo$ sam logs -n HelloWorldFunction --stack-name lambda-dynamo --tail
```
...
```bash
aws cloudformation delete-stack --stack-name lambda-dynamo
```
